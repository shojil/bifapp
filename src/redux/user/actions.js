...
/**
  ...
  */
async function saveCredentialsToStorage(username = '', password = '') {
...
/**
  ...
  * Login to Firebase with Username/Password
  */
export function login(formData = {}, verifyEmail = false) {
  ...
  return async (dispatch) => {
    ...
    if (!username || !password) {
      ...
      if (!username) username = credsFromStorage.username;
      if (!password) password = credsFromStorage.password;
    }

    // Update Login Creds in AsyncStorage
    if (username && password) saveCredentialsToStorage(username, password);
    ...
    /*return Firebase.auth()
      ...
      return dispatch({
          type: 'USER_LOGIN',
          data: {uid:1112, email: "testing@gmail.com",
          emailVerified: "yes"},
        });
  };
}
